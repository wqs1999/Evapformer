...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        self.attns_file_index = 1  # 初始化attns文件的序号
        self.norms_file_index = 1  # 初始化norms文件的序号
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        if self.training == True:
            if self.training:  # 只有在模型训练时才执行存储
                #attn_file_path = os.path.join('./attns', f'attn_{self.attns_file_index}.npz')
                #norms_file_path = os.path.join('./norms', f'norms_{self.norms_file_index}.npz')

                #np.savez(attn_file_path, attn=attn.detach().cpu().numpy())
                #np.savez(norms_file_path, norms=norms.detach().cpu().numpy())

                self.attns_file_index += 1
                self.norms_file_index += 1
